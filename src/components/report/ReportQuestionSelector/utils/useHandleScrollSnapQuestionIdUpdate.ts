...
export const useHandleScrollSnapQuestionIdUpdate = ({
  ...
  const handleScrollSnapChange = useCallback(
    (e: $TSFixMe) => {
      if (
        !e?.snapTargetBlock ||
        !e?.snapTargetBlock?.dataset?.[QUESTION_ELEMENT_DATA_ID]
      )
        return;
  ...
  useLayoutEffect(() => {
    ...
    questionsWrapper.addEventListener(
      "scrollsnapchange",
      handleScrollSnapChange,
    );
    return () => {
      questionsWrapper.removeEventListener(
        "scrollsnapchange",
        handleScrollSnapChange,
      );
    };
  }, [handleScrollSnapChange, questionsWrapperRef]);
