...
export const getGroupedAnswers = (answers: ReportAnswer[]): ReportAnswer[] => {
  ...
  for (const answer of answers) {
    const groupNameAnswers = Object.entries(STANDARD_GROUP_ANSWERS).find(
      ([, values]) => values.includes(answer.text),
    );
    if (!groupNameAnswers) {
  ...
  return Object.entries(groupedAnswers).map(([groupName, groupAnswers]) => ({
    id: groupAnswers.reduce(
      (ids, answer): string => [ids, answer.id].join("-"),
      "",
    ),
    text: groupName,
